package com.labdogstudio.tutorial.tut9;
...
public class RayPickingTest extends BaseScreen {
	...
	public RayPickingTest(Game game) {
		super(game);
	}
	...
	/*
	 * public int getObject(int screenX, int screenY) { Ray ray =
	 * cam.getPickRay(screenX, screenY); int result = -1; float distance = -1;
	 * for (int i = 0; i < instances.size; ++i) { final GameObject instance =
	 * instances.get(i); instance.transform.getTranslation(position);
	 * position.add(instance.center); float dist2 = ray.origin.dst2(position);
	 * if (distance >= 0f && dist2 > distance) continue; if
	 * (Intersector.intersectRaySphere(ray, position, instance.radius, null)) {
	 * result = i; distance = dist2; } } return result; }
	 */
